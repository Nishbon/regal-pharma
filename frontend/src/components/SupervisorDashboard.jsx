...
const SupervisorDashboard = () => {
  ...
  // PDF EXPORT FUNCTIONS - UPDATED WITH PROFESSIONAL DESIGN
  const exportTeamPDF = () => {
    try {
      const doc = new jsPDF()
      
      // Add Times New Roman font (default in jsPDF)
      doc.setFont('Times', 'normal')
      
      // Company Header Section
      doc.setFontSize(24)
      doc.setTextColor(20, 108, 148) // Deep blue
      doc.setFont('Times', 'bold')
      doc.text('REGAL PHARMACEUTICALS', 105, 25, { align: 'center' })
      
      doc.setFontSize(12)
      doc.setTextColor(59, 130, 246) // Light blue
      doc.setFont('Times', 'normal')
      doc.text('Rwanda • Excellence in Healthcare', 105, 33, { align: 'center' })
      ...
      doc.setFont('Times', 'bold')
      doc.text('TEAM PERFORMANCE REPORT', 105, 50, { align: 'center' })
      doc.text('Supervisor Dashboard', 105, 58, { align: 'center' })
      ...
      const periodLabels = {
        ...
        '3months': 'Quarterly (Last 90 Days)'
      }
      
      const details = [
        `Period: ${periodLabels[timeRange] || 'Monthly (Last 30 Days)'}`,
        ...
        `Generated by: ${user?.name || 'Supervisor'}`,
        `Report ID: RP-${new Date().getFullYear()}${(new Date().getMonth() + 1).toString().padStart(2, '0')}-${Math.floor(Math.random() * 10000)}`
      ]
      
      details.forEach((detail, index) => {
        doc.text(detail, 20, 70 + (index * 7))
      })
      ...
      if (teamReports.length === 0) {
        ...
        doc.text('No team performance data available for the selected period', 105, 110, { align: 'center' })
      } else {
        ...
        doc.setFont('Times', 'bold')
        doc.text('EXECUTIVE SUMMARY', 30, 100)
        ...
        doc.setFont('Times', 'normal')
        doc.text(`• Total Team Members: ${teamReports.length}`, 30, 108)
        doc.text(`• Total Reports Submitted: ${totals.reports}`, 30, 115)
        doc.text(`• Total Revenue Generated: RWF ${totals.value.toLocaleString()}`, 30, 122)
        doc.text(`• Average Revenue per Report: RWF ${Math.round(totals.value / Math.max(totals.reports, 1)).toLocaleString()}`, 105, 122)
        doc.text(`• Total Orders: ${totals.orders}`, 105, 115)
        doc.text(`• Total Doctor Visits: ${totals.doctors}`, 105, 108)
        
        // Team Performance Table
        doc.setFontSize(14)
        doc.setTextColor(20, 108, 148)
        doc.setFont('Times', 'bold')
        doc.text('TEAM PERFORMANCE DETAILS', 20, 145)
        ...
        autoTable(doc, {
          head: [['#', 'Medical Representative', 'Region', 'Reports', 'Doctors Visited', 'Orders', 'Total Revenue', 'Avg/Report']],
          body: tableData,
          startY: 150,
          theme: 'grid',
          headStyles: { 
        ...
        doc.setTextColor(20, 108, 148)
        doc.setFont('Times', 'bold')
        doc.text('TOP PERFORMERS', 20, finalY + 15)
        
        const top3 = [...teamReports]
          .sort((a, b) => (b.total_value || 0) - (a.total_value || 0))
          .slice(0, 3)
        
        top3.forEach((member, index) => {
          const yPos = finalY + 25 + (index * 15)
          
          // Rank badge
          doc.setFillColor(
            index === 0 ? [255, 215, 0] : // Gold
            index === 1 ? [192, 192, 192] : // Silver
            [205, 127, 50] // Bronze
          )
          doc.roundedRect(25, yPos - 5, 10, 10, 5, 5, 'F')
          doc.setTextColor(255, 255, 255)
          doc.setFont('Times', 'bold')
          doc.text(`${index + 1}`, 30, yPos + 1, { align: 'center' })
          
          // Name and details
          doc.setTextColor(0, 0, 0)
          doc.setFont('Times', 'normal')
          doc.text(`${member.user_name} - ${member.region || 'N/A'}`, 40, yPos)
          doc.text(`Revenue: RWF ${(member.total_value || 0).toLocaleString()} | Reports: ${member.reports_count || 0}`, 40, yPos + 5)
        })
        
        // Performance Analysis
        const analysisY = finalY + 75
        ...
        const reportingRate = Math.round((teamReports.length / Math.max(activeMedReps.length, 1)) * 100)
        const avgDoctorsPerReport = Math.round(totals.doctors / Math.max(totals.reports, 1))
        const avgOrdersPerReport = Math.round(totals.orders / Math.max(totals.reports, 1))
        const conversionRate = Math.round((totals.orders / Math.max(totals.doctors, 1)) * 100)
        
        doc.setFontSize(10)
        doc.setTextColor(0, 0, 0)
        doc.setFont('Times', 'normal')
        doc.text(`• Reporting Rate: ${reportingRate}% of active medical representatives`, 25, analysisY + 10)
        doc.text(`• Average Doctors per Report: ${avgDoctorsPerReport}`, 25, analysisY + 17)
        doc.text(`• Average Orders per Report: ${avgOrdersPerReport}`, 25, analysisY + 24)
        doc.text(`• Conversion Rate (Orders/Doctors): ${conversionRate}%`, 25, analysisY + 31)
        ...
        doc.setTextColor(150, 150, 150)
        doc.setFont('Times', 'italic')
        doc.text(`Confidential - Regal Pharmaceuticals Rwanda • Page 1 of 1 • Generated on ${new Date().toISOString()}`, 105, pageHeight - 10, { align: 'center' })
      }

      // Save PDF
      const fileName = `Regal-Pharma-Team-Performance-${timeRange}-${new Date().toISOString().split('T')[0]}.pdf`
      doc.save(fileName)
  ...
  const exportMemberPDF = (memberId) => {
    try {
      ...
      doc.setTextColor(0, 0, 0)
      doc.setFont('Times', 'bold')
      doc.text('INDIVIDUAL PERFORMANCE REPORT', 105, 50, { align: 'center' })
      
      // Medical Representative Details
      doc.setFontSize(16)
      doc.setTextColor(20, 108, 148)
      doc.text(member.user_name.toUpperCase(), 105, 60, { align: 'center' })
      ...
      doc.text(`Period: ${periodLabels[timeRange] || 'Monthly (Last 30 Days)'}`, 25, 75)
      doc.text(`Region: ${member.region || 'Not Specified'}`, 25, 82)
      doc.text(`Report ID: RP-IND-${member.user_id.slice(-6).toUpperCase()}`, 25, 89)
      doc.text(`Email: ${member.user_email || 'Not Provided'}`, 110, 75)
      doc.text(`Generated: ${new Date().toLocaleDateString()}`, 110, 82)
      doc.text(`Generated by: ${user?.name || 'Supervisor'}`, 110, 89)
      ...
      const summaryData = [
        ['Total Reports Submitted', member.reports_count || 0],
        ...
        ['Total Orders Placed', member.total_orders || 0],
        ['Total Revenue Generated', `RWF ${(member.total_value || 0).toLocaleString()}`],
        ['Average Doctors per Report', Math.round((member.total_doctors || 0) / Math.max(member.reports_count, 1))],
        ['Average Orders per Report', Math.round((member.total_orders || 0) / Math.max(member.reports_count, 1))],
        ['Average Revenue per Report', `RWF ${Math.round((member.total_value || 0) / Math.max(member.reports_count, 1)).toLocaleString()}`]
      ]
      ...
      if (member.reports && member.reports.length > 0) {
        const recentY = doc.lastAutoTable?.finalY + 15 || 140
        
        doc.setFontSize(14)
        doc.setTextColor(20, 108, 148)
        doc.setFont('Times', 'bold')
        doc.text('RECENT ACTIVITY (Last 10 Reports)', 20, recentY)
        ...
        const reportData = member.reports.slice(0, 10).map(report => {
          ...
          return [
            ...
            (report.summary || 'No notes').substring(0, 35) + ((report.summary || '').length > 35 ? '...' : '')
          ]
        })
        ...
        // Performance Metrics
        const metricsY = doc.lastAutoTable?.finalY + 20 || 200
        ...
        const reportsCount = member.reports_count || 0
        const totalValue = member.total_value || 0
        const avgRevenue = Math.round(totalValue / Math.max(reportsCount, 1))
        const avgDoctors = Math.round((member.total_doctors || 0) / Math.max(reportsCount, 1))
        const avgOrders = Math.round((member.total_orders || 0) / Math.max(reportsCount, 1))
        
        doc.text(`• Reports per day: ${(reportsCount / (timeRange === 'week' ? 7 : timeRange === 'month' ? 30 : 90)).toFixed(1)}`, 25, metricsY + 10)
        doc.text(`• Average revenue per day: RWF ${Math.round(totalValue / (timeRange === 'week' ? 7 : timeRange === 'month' ? 30 : 90)).toLocaleString()}`, 25, metricsY + 17)
        doc.text(`• Doctor conversion rate: ${Math.round(((member.total_orders || 0) / Math.max(member.total_doctors, 1)) * 100)}%`, 105, metricsY + 10)
        doc.text(`• Order value per order: RWF ${Math.round(totalValue / Math.max(member.total_orders, 1)).toLocaleString()}`, 105, metricsY + 17)
      } else {
        // No reports message
        const noReportsY = doc.lastAutoTable?.finalY + 20 || 140
        doc.setFontSize(12)
        doc.setTextColor(150, 150, 150)
        doc.setFont('Times', 'italic')
        doc.text('No individual reports available for this period', 105, noReportsY, { align: 'center' })
      }
      ...
      doc.setTextColor(150, 150, 150)
      doc.setFont('Times', 'italic')
      doc.text(`Confidential - Regal Pharmaceuticals Rwanda • Page 1 of 1 • Employee ID: ${member.user_id.slice(-8).toUpperCase()}`, 105, pageHeight - 10, { align: 'center' })
  ...
  const exportExecutiveSummary = () => {
    try {
      ...
      doc.setTextColor(59, 130, 246)
      doc.setFont('Times', 'normal')
      doc.text('Rwanda - Excellence in Healthcare Solutions', 105, 35, { align: 'center' })
      ...
      doc.setTextColor(0, 0, 0)
      doc.setFont('Times', 'bold')
      doc.text('EXECUTIVE PERFORMANCE SUMMARY', 105, 55, { align: 'center' })
      ...
      const periodLabels = {
        ...
        '3months': 'Quarterly Report (Last 90 Days)'
      }
      
      const details = [
        `Report Period: ${periodLabels[timeRange] || 'Monthly Report'}`,
        ...
        `Prepared by: ${user?.name || 'Supervisor'} | ${user?.email || 'supervisor@regalpharma.rw'}`,
        `Document ID: RP-EXEC-${new Date().getFullYear()}${(new Date().getMonth() + 1).toString().padStart(2, '0')}-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`
      ]
      
      details.forEach((detail, index) => {
        doc.text(detail, 20, 65 + (index * 7))
      })
      ...
      doc.setDrawColor(20, 108, 148)
      doc.rect(20, 100, 170, 50)
      
      doc.setFontSize(16)
      doc.setTextColor(20, 108, 148)
      doc.setFont('Times', 'bold')
      doc.text('AT A GLANCE', 105, 110, { align: 'center' })
      ...
      const reportingRate = Math.round((teamReports.length / Math.max(activeMedReps.length, 1)) * 100)
      const avgRevenuePerRep = Math.round(totals.value / Math.max(teamReports.length, 1))
      ...
      const glanceData = [
        `• Active Medical Representatives: ${activeMedReps.length}`,
        `• Reporting Rate: ${reportingRate}%`,
        `• Total Reports Submitted: ${totals.reports}`,
        `• Total Revenue Generated: RWF ${totals.value.toLocaleString()}`,
        `• Average Revenue per Representative: RWF ${avgRevenuePerRep.toLocaleString()}`,
        `• Total Doctor Visits: ${totals.doctors}`,
        `• Total Orders Placed: ${totals.orders}`,
        `• Average Orders per Report: ${Math.round(totals.orders / Math.max(totals.reports, 1))}`
      ]
      
      // Two columns layout
      glanceData.slice(0, 4).forEach((item, index) => {
        doc.text(item, 25, 120 + (index * 7))
      })
      
      glanceData.slice(4).forEach((item, index) => {
        doc.text(item, 105, 120 + (index * 7))
      })
      ...
      const metricsData = [
        ...
        ['Reporting Compliance', `${reportingRate}%`, '90%', reportingRate >= 90 ? '✓ Achieved' : '⚠ Needs Improvement'],
        ['Average Revenue per Report', `RWF ${Math.round(totals.value / Math.max(totals.reports, 1)).toLocaleString()}`, 'RWF 100,000', Math.round(totals.value / Math.max(totals.reports, 1)) >= 100000 ? '✓ Achieved' : '⚠ Below Target'],
        ['Doctor Visit Efficiency', `${Math.round(totals.doctors / Math.max(totals.reports, 1))} per report`, '5 per report', Math.round(totals.doctors / Math.max(totals.reports, 1)) >= 5 ? '✓ Achieved' : '⚠ Below Target'],
        ['Order Conversion Rate', `${Math.round((totals.orders / Math.max(totals.doctors, 1)) * 100)}%`, '25%', Math.round((totals.orders / Math.max(totals.doctors, 1)) * 100) >= 25 ? '✓ Achieved' : '⚠ Below Target'],
        ['Revenue Growth', `${Math.round((totals.value / Math.max(totals.reports, 1)) / 1000)}K/report`, '150K/report', Math.round(totals.value / Math.max(totals.reports, 1)) >= 150000 ? '✓ Achieved' : '⚠ Below Target']
      ]
      ...
      autoTable(doc, {
        ...
        columnStyles: {
          0: { cellWidth: 60, font: 'Times' },
          1: { cellWidth: 40, font: 'Times' },
          2: { cellWidth: 40, font: 'Times' },
          3: { 
            cellWidth: 50, 
            font: 'Times',
            fontStyle: 'bold'
        ...
        didDrawCell: (data) => {
          ...
          } else if (data.column.index === 3 && data.cell.raw.includes('⚠')) {
      ...
      // Top Performers Section
      const topPerfY = doc.lastAutoTable?.finalY + 20 || 220
      
      doc.setFontSize(14)
      doc.setTextColor(20, 108, 148)
      doc.setFont('Times', 'bold')
      doc.text('TOP PERFORMANCE RECOGNITION', 20, topPerfY)
      ...
      if (teamReports.length > 0) {
        ...
        top3.forEach((member, index) => {
          const yPos = topPerfY + 15 + (index * 25)
          
          // Performance badge with color based on rank
          const rankColors = [
            [255, 215, 0],    // Gold
            [192, 192, 192],  // Silver
            [205, 127, 50]    // Bronze
          ...
          doc.roundedRect(25, yPos - 8, 12, 12, 6, 6, 'F')
          doc.setTextColor(255, 255, 255)
          doc.setFont('Times', 'bold')
          doc.text(`${index + 1}`, 31, yPos - 1, { align: 'center' })
          
          // Member details
          doc.setTextColor(0, 0, 0)
          doc.setFontSize(12)
          doc.setFont('Times', 'bold')
          doc.text(member.user_name, 40, yPos)
          doc.setFontSize(10)
          doc.setFont('Times', 'normal')
          doc.text(`Region: ${member.region || 'N/A'} | Reports: ${member.reports_count || 0}`, 40, yPos + 5)
          doc.text(`Revenue: RWF ${(member.total_value || 0).toLocaleString()} | Orders: ${member.total_orders || 0}`, 40, yPos + 11)
        })
        
        // Performance Insights
        const insightsY = topPerfY + 100
        
        doc.setFontSize(14)
        doc.setTextColor(20, 108, 148)
        doc.setFont('Times', 'bold')
        doc.text('PERFORMANCE INSIGHTS', 20, insightsY)
        
        const topPerformer = top3[0]
        const topPerformerShare = Math.round((topPerformer.total_value || 0) / Math.max(totals.value, 1) * 100)
        const avgReportsPerActive = Math.round(totals.reports / Math.max(activeMedReps.length, 1))
        const teamAvgRevenue = Math.round(totals.value / Math.max(teamReports.length, 1))
        
        const insights = [
          `Top performer ${topPerformer.user_name} contributed ${topPerformerShare}% of total team revenue`,
          `Average reports per active representative: ${avgReportsPerActive} reports`,
          `Team average revenue per representative: RWF ${teamAvgRevenue.toLocaleString()}`,
          `Conversion efficiency: ${Math.round((totals.orders / Math.max(totals.doctors, 1)) * 100)}% of doctor visits converted to orders`,
          `Revenue per doctor visit: RWF ${Math.round(totals.value / Math.max(totals.doctors, 1)).toLocaleString()}`
        ]
        ...
        insights.forEach((insight, index) => {
          doc.text(`• ${insight}`, 25, insightsY + 10 + (index * 7))
        })
        
        // Strategic Recommendations
        const recY = insightsY + 50
        
        doc.setFillColor(255, 250, 240)
        doc.rect(20, recY, 170, 40, 'F')
        doc.setDrawColor(245, 158, 11)
        doc.rect(20, recY, 170, 40)
        
        doc.setFontSize(14)
        doc.setTextColor(245, 158, 11)
        doc.setFont('Times', 'bold')
        doc.text('STRATEGIC RECOMMENDATIONS', 30, recY + 15)
        
        const recommendations = [
          '1. Implement recognition program for top 3 performers',
          '2. Provide targeted training for representatives below average',
          '3. Review and optimize regional coverage strategies',
          '4. Consider incentives for improved reporting compliance'
        ]
        ...
        recommendations.forEach((rec, index) => {
          doc.text(rec, 25, recY + 25 + (index * 7))
        })
      }
      ...
      doc.setTextColor(150, 150, 150)
      doc.setFont('Times', 'italic')
      doc.text(`Confidential - Regal Pharmaceuticals Rwanda Executive Report • Page 1 of 1 • Valid until ${new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]}`, 105, pageHeight - 10, { align: 'center' })

      doc.save(`Regal-Pharma-Executive-Summary-${timeRange}-${new Date().toISOString().split('T')[0]}.pdf`)
      setSuccessMessage('Executive summary exported successfully!')
    } catch (error) {
      console.error('Error exporting executive summary:', error)
      setError('Failed to export executive summary')
    }
  }

  const exportAllMemberReports = () => {
    if (teamReports.length === 0) {
      setError('No team members found to export')
      return
    }

    setSuccessMessage(`Exporting ${teamReports.length} member reports...`)
    
    teamReports.forEach((member, index) => {
      setTimeout(() => {
        exportMemberPDF(member.user_id)
      ...
      <div className="actions-card">
        ...
        <div className="actions-grid">
          ...
          <ActionButton 
            ...
            onClick={exportAllMemberReports}
            color="#10b981"
          />
          <ActionButton 
            icon="📋"
            title="Export Executive Summary" 
            description="Export executive summary report with key metrics and recommendations"
    ...
    <div className="member-info">
      <div className="rank-badge" style={{
        background: rank <= 3 ? 
          ...
          'linear-gradient(135deg, #146C94 0%, #0A4D68 100%)'
      }}>
        {rank}
      </div>
...
// CSS Styles - UPDATED WITH PROFESSIONAL COLORS
const styles = `
...
/* Header */
.supervisor-header {
  background: linear-gradient(135deg, #0A4D68 0%, #146C94 100%);
  color: white;
  padding: 40px;
  border-radius: 16px;
  margin-bottom: 30px;
  box-shadow: 0 10px 30px rgba(20, 108, 148, 0.25);
...
.card-header::after {
  ...
  width: 60px;
  height: 2px;
  background: linear-gradient(90deg, #146C94 0%, #3b82f6 100%);
}
...
.performer-icon {
  width: 80px;
  height: 80px;
  background: linear-gradient(135deg, #146C94 0%, #0A4D68 100%);
  border-radius: 50%;
  display: flex;
  align-items: center;
  justify-content: center;
  ...
  color: white;
  box-shadow: 0 5px 15px rgba(20, 108, 148, 0.3);
}
...
.time-select:focus {
  outline: none;
  border-color: #146C94;
  box-shadow: 0 0 0 3px rgba(20, 108, 148, 0.1);
}
...
.team-stats-card .card-header::after {
  background: linear-gradient(90deg, #0A4D68 0%, #146C94 100%);
}
