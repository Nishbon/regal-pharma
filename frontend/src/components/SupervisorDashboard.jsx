...
const SupervisorDashboard = () => {
  ...
  const exportExecutiveSummary = () => {
    try {
      ...
      // Title
      doc.setFontSize(20)
      doc.text('Executive Performance Summary', 20, 20)
      ...
      const topPerformer = teamReports.length > 0 ? teamReports.reduce((top, current) => 
        (current.total_value || 0) > (top.total_value || 0) ? current : top
      ) : null

      const metricsData = [
        ['Active Medical Representatives', activeMedReps.length],
        ['Reporting Team Members', teamReports.length],
        ['Reporting Rate', `${Math.round((teamReports.length / Math.max(activeMedReps.length, 1)) * 100)}%`],
        ...
        ['Average Revenue per Report', `RWF ${Math.round(totals.value / Math.max(totals.reports, 1)).toLocaleString()}`],
        ['Average Revenue per Team Member', `RWF ${Math.round(totals.value / Math.max(teamReports.length, 1)).toLocaleString()}`]
      ]
      ...
      // Top Performers
      if (teamReports.length > 0) {
        const topPerformersY = doc.lastAutoTable?.finalY + 15 || 130
        doc.setFontSize(14)
        doc.text('Top Performers', 20, topPerformersY)
        
        const top3 = [...teamReports]
          .sort((a, b) => (b.total_value || 0) - (a.total_value || 0))
          .slice(0, 3)
        
        const performersData = top3.map((member, index) => [
          index + 1,
          member.user_name,
          member.region || 'N/A',
          `RWF ${(member.total_value || 0).toLocaleString()}`,
        ...
        autoTable(doc, {
          head: [['Rank', 'Name', 'Region', 'Revenue', 'Reports']],
          body: performersData,
        ...
        if (topPerformer) {
          const insightsY = doc.lastAutoTable?.finalY + 15 || 160
          doc.setFontSize(14)
          doc.text('Performance Insights', 20, insightsY)
          
          const insights = [
            `Top performer ${topPerformer.user_name} generated ${Math.round((topPerformer.total_value || 0) / Math.max(totals.value, 1) * 100)}% of total team revenue`,
            `Average reports per active member: ${Math.round(totals.reports / Math.max(activeMedReps.length, 1))}`,
            `Average orders per report: ${Math.round(totals.orders / Math.max(totals.reports, 1))}`,
            `Conversion rate (orders/doctor visits): ${Math.round((totals.orders / Math.max(totals.doctors, 1)) * 100)}%`
          ]
          
          insights.forEach((insight, index) => {
            doc.setFontSize(10)
            doc.text(`• ${insight}`, 25, insightsY + 10 + (index * 6))
          })
        }
      }

      // Recommendations
      const recommendationsY = doc.lastAutoTable?.finalY + 30 || 200
      doc.setFontSize(14)
      doc.text('Recommendations', 20, recommendationsY)
      
      const recommendations = [
        'Focus on increasing reporting rate among all team members',
        'Provide additional training for members with low doctor visit counts',
        'Implement incentives for top performers to maintain momentum',
        'Review regions with lower performance for targeted support'
      ]
      
      recommendations.forEach((rec, index) => {
        doc.setFontSize(10)
        doc.text(`${index + 1}. ${rec}`, 25, recommendationsY + 10 + (index * 6))
      })

      doc.save(`Executive-Summary-${timeRange}-${new Date().toISOString().split('T')[0]}.pdf`)
